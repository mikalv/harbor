...
ETCD_FQDN_HOSTNAME="${ETCD_HOSTNAME}.${POD_NAMESPACE}.svc.${OS_DOMAIN}"
echo "${OS_DISTRO}: Container application launch"
################################################################################
echo "$POD_IP $ETCD_FQDN_HOSTNAME" >> /etc/hosts
exec etcd \
      --name=default \
      --listen-peer-urls="https://${ETCD_FQDN_HOSTNAME}:${ETCD_PEERS_PORT}" \
      --initial-advertise-peer-urls="https://${ETCD_FQDN_HOSTNAME}:${ETCD_PEERS_PORT}" \
      --initial-cluster="default=https://${ETCD_FQDN_HOSTNAME}:${ETCD_PEERS_PORT}" \
      --listen-client-urls="https://${ETCD_FQDN_HOSTNAME}:${ETCD_PORT}" \
      --advertise-client-urls="https://${ETCD_FQDN_HOSTNAME}:${ETCD_PORT}" \
