...
################################################################################
/opt/harbor/config-elasticsearch.sh


echo "${OS_DISTRO}: Configuring Permissions"
################################################################################
chown -R ${APP_USER}:${APP_USER} ${ELS_CONFIG_DIR}
chown -R ${APP_USER}:${APP_USER} ${ELS_DATA_DIR}


echo "${OS_DISTRO}: Moving pod configs into place"
################################################################################
cp -rfav ${ELS_CONFIG_DIR}/* /pod${ELS_CONFIG_DIR}/
