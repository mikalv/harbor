...
for OS_SERVICE_NAME in heat heat-cfn; do
  ...
  if [ "$OS_SERVICE_NAME" == "heat-cfn" ]; then
    ...
    OS_SVC_ENDPOINT_INTERNAL="https://${HEAT_API_CFN_SERVICE_HOST_SVC}/v1"
    ...
    OS_SERVICE_DESC="${OS_DOMAIN}: ${OS_SERVICE_NAME} (${OS_SERVICE_TYPE}) service"
    OS_SVC_ENDPOINT_INTERNAL="https://${HEAT_API_SERVICE_HOST_SVC}/v1/\$(project_id)s"
  ...
  for OS_SVC_ENDPOINT in ${OS_SVC_ENDPOINTS}; do
    ...
    OS_ENDPOINT_ID=$( openstack endpoint list  -f csv --quote none | \
                      grep "^[a-z0-9]*,${AUTH_HEAT_KEYSTONE_REGION},${OS_SERVICE_NAME},${OS_SERVICE_TYPE},True,${CURRENT_OS_SVC_ENDPOINT_IFACE}," | \
                      awk -F ',' '{ print $1 }' )
    ...
    if [[ "${OS_ENDPOINT_UPDATE}" == "True" ]]; then
      OS_ENDPOINT_ID=$( openstack endpoint create -f value -c id \
        --region="${AUTH_HEAT_KEYSTONE_REGION}" \
        "${OS_SERVICE_ID}" \
