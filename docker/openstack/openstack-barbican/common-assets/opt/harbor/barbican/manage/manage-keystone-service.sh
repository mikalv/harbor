...
check_required_vars OS_DOMAIN \
                    ...
                    BARBICAN_API_SERVICE_HOST_SVC \
                    BARBICAN_API_SERVICE_HOST \
                    BARBICAN_API_SVC_PORT


################################################################################
for OS_SERVICE_NAME in barbican; do
  OS_SVC_ENDPOINTS="PUBLIC ADMIN INTERNAL"

  if [ "$OS_SERVICE_NAME" == "barbican" ]; then
    OS_SERVICE_TYPE="key-manager"
    OS_SERVICE_DESC="${OS_DOMAIN}: ${OS_SERVICE_NAME} (${OS_SERVICE_TYPE}) service"
    OS_SVC_ENDPOINT_INTERNAL="https://${BARBICAN_API_SERVICE_HOST_SVC}"
    OS_SVC_ENDPOINT_ADMIN="https://${BARBICAN_API_SERVICE_HOST_SVC}:${BARBICAN_API_SVC_PORT}"
    OS_SVC_ENDPOINT_PUBLIC="https://${BARBICAN_API_SERVICE_HOST}"
  fi
  ...
  for OS_SVC_ENDPOINT in ${OS_SVC_ENDPOINTS}; do
    ...
    OS_ENDPOINT_ID=$( openstack endpoint list  -f csv --quote none | \
                      grep "^[a-z0-9]*,${AUTH_BARBICAN_KEYSTONE_REGION},${OS_SERVICE_NAME},${OS_SERVICE_TYPE},True,${CURRENT_OS_SVC_ENDPOINT_IFACE}," | \
                      awk -F ',' '{ print $1 }' )
    ...
    if [[ "${OS_ENDPOINT_UPDATE}" == "True" ]]; then
      OS_ENDPOINT_ID=$( openstack endpoint create -f value -c id \
        --region="${AUTH_BARBICAN_KEYSTONE_REGION}" \
        "${OS_SERVICE_ID}" \
