...
for OS_SERVICE_NAME in magnum; do
  ...
  if [ "$OS_SERVICE_NAME" == "magnum" ]; then
    ...
    OS_SVC_ENDPOINT_INTERNAL="https://${MAGNUM_API_SERVICE_HOST_SVC}"
    OS_SVC_ENDPOINT_ADMIN="https://${MAGNUM_API_SERVICE_HOST_SVC}:${MAGNUM_API_SVC_PORT}"
    OS_SVC_ENDPOINT_PUBLIC="https://${MAGNUM_API_SERVICE_HOST}"
  fi
