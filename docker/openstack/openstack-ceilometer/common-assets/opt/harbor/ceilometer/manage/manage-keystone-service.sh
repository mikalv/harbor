...
check_required_vars OS_DOMAIN \
                    ...
                    CEILOMETER_API_SERVICE_HOST_SVC \
                    CEILOMETER_API_SERVICE_HOST \
                    CEILOMETER_API_SVC_PORT


################################################################################
for OS_SERVICE_NAME in ceilometer; do
  OS_SVC_ENDPOINTS="PUBLIC ADMIN INTERNAL"

  if [ "$OS_SERVICE_NAME" == "ceilometer" ]; then
    OS_SERVICE_TYPE="workflowv2"
    OS_SERVICE_DESC="${OS_DOMAIN}: ${OS_SERVICE_NAME} (${OS_SERVICE_TYPE}) service"
    OS_SVC_ENDPOINT_INTERNAL="https://${CEILOMETER_API_SERVICE_HOST_SVC}/v2"
    OS_SVC_ENDPOINT_ADMIN="https://${CEILOMETER_API_SERVICE_HOST_SVC}:${CEILOMETER_API_SVC_PORT}/v2"
    OS_SVC_ENDPOINT_PUBLIC="https://${CEILOMETER_API_SERVICE_HOST}/v2"
  fi
  ...
  for OS_SVC_ENDPOINT in ${OS_SVC_ENDPOINTS}; do
    ...
    OS_ENDPOINT_ID=$( openstack endpoint list  -f csv --quote none | \
                      grep "^[a-z0-9]*,${AUTH_CEILOMETER_KEYSTONE_REGION},${OS_SERVICE_NAME},${OS_SERVICE_TYPE},True,${CURRENT_OS_SVC_ENDPOINT_IFACE}," | \
                      awk -F ',' '{ print $1 }' )
    ...
    if [[ "${OS_ENDPOINT_UPDATE}" == "True" ]]; then
      OS_ENDPOINT_ID=$( openstack endpoint create -f value -c id \
        --region="${AUTH_CEILOMETER_KEYSTONE_REGION}" \
        "${OS_SERVICE_ID}" \
