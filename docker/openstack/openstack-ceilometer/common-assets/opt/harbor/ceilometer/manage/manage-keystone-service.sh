...
for OS_SERVICE_NAME in ceilometer; do
  ...
  if [ "$OS_SERVICE_NAME" == "ceilometer" ]; then
    ...
    OS_SVC_ENDPOINT_INTERNAL="https://${CEILOMETER_API_SERVICE_HOST_SVC}"
    OS_SVC_ENDPOINT_ADMIN="https://${CEILOMETER_API_SERVICE_HOST_SVC}:${CEILOMETER_API_SVC_PORT}"
    OS_SVC_ENDPOINT_PUBLIC="https://${CEILOMETER_API_SERVICE_HOST}"
  fi
