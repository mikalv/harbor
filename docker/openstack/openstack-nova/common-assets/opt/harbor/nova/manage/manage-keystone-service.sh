...
for SERVICE_TYPE in compute; do
  ...
  for OS_SVC_ENDPOINT in ${OS_SVC_ENDPOINTS}; do
    ...
    OS_ENDPOINT_ID=$( openstack endpoint list  -f csv --quote none | \
                      grep "^[a-z0-9]*,${AUTH_NOVA_KEYSTONE_REGION},${OS_SERVICE_NAME},${OS_SERVICE_TYPE},True,${CURRENT_OS_SVC_ENDPOINT_IFACE}," | \
                      awk -F ',' '{ print $1 }' )
    ...
    if [[ "${OS_ENDPOINT_UPDATE}" == "True" ]]; then
      OS_ENDPOINT_ID=$( openstack endpoint create -f value -c id \
        --region="${AUTH_NOVA_KEYSTONE_REGION}" \
        "${OS_SERVICE_ID}" \
