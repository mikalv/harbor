...
check_required_vars OS_DOMAIN \
                    ...
                    MURANO_API_SERVICE_HOST_SVC \
                    MURANO_API_SERVICE_HOST \
                    MURANO_API_SVC_PORT


################################################################################
for OS_SERVICE_NAME in murano; do
  OS_SVC_ENDPOINTS="PUBLIC ADMIN INTERNAL"

  if [ "$OS_SERVICE_NAME" == "murano" ]; then
    OS_SERVICE_TYPE="application-catalog"
    OS_SERVICE_DESC="${OS_DOMAIN}: ${OS_SERVICE_NAME} (${OS_SERVICE_TYPE}) service"
    OS_SVC_ENDPOINT_INTERNAL="https://${MURANO_API_SERVICE_HOST_SVC}:${MURANO_API_SVC_PORT}"
    OS_SVC_ENDPOINT_ADMIN="https://${MURANO_API_SERVICE_HOST_SVC}:${MURANO_API_SVC_PORT}"
    OS_SVC_ENDPOINT_PUBLIC="https://${MURANO_API_SERVICE_HOST}"
  fi
  ...
  for OS_SVC_ENDPOINT in ${OS_SVC_ENDPOINTS}; do
    ...
    OS_ENDPOINT_ID=$( openstack endpoint list  -f csv --quote none | \
                      grep "^[a-z0-9]*,${AUTH_MURANO_KEYSTONE_REGION},${OS_SERVICE_NAME},${OS_SERVICE_TYPE},True,${CURRENT_OS_SVC_ENDPOINT_IFACE}," | \
                      awk -F ',' '{ print $1 }' )
    ...
    if [[ "${OS_ENDPOINT_UPDATE}" == "True" ]]; then
      OS_ENDPOINT_ID=$( openstack endpoint create -f value -c id \
        --region="${AUTH_MURANO_KEYSTONE_REGION}" \
        "${OS_SERVICE_ID}" \
