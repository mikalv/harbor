...
for OS_SERVICE_NAME in zun; do
  ...
  if [ "$OS_SERVICE_NAME" == "zun" ]; then
    ...
    OS_SVC_ENDPOINT_INTERNAL="https://${ZUN_API_SERVICE_HOST_SVC}"
    OS_SVC_ENDPOINT_ADMIN="https://${ZUN_API_SERVICE_HOST_SVC}:${ZUN_API_SVC_PORT}"
    OS_SVC_ENDPOINT_PUBLIC="https://${ZUN_API_SERVICE_HOST}"
  fi
