...
check_required_vars OS_DOMAIN \
                    ...
                    ZUN_API_SERVICE_HOST_SVC \
                    ZUN_API_SERVICE_HOST \
                    ZUN_API_SVC_PORT


################################################################################
for OS_SERVICE_NAME in zun; do
  OS_SVC_ENDPOINTS="PUBLIC ADMIN INTERNAL"

  if [ "$OS_SERVICE_NAME" == "zun" ]; then
    OS_SERVICE_TYPE="container"
    OS_SERVICE_DESC="${OS_DOMAIN}: ${OS_SERVICE_NAME} (${OS_SERVICE_TYPE}) service"
    OS_SVC_ENDPOINT_INTERNAL="https://${ZUN_API_SERVICE_HOST_SVC}/v1"
    OS_SVC_ENDPOINT_ADMIN="https://${ZUN_API_SERVICE_HOST_SVC}:${ZUN_API_SVC_PORT}/v1"
    OS_SVC_ENDPOINT_PUBLIC="https://${ZUN_API_SERVICE_HOST}/v1"
  fi
  ...
  for OS_SVC_ENDPOINT in ${OS_SVC_ENDPOINTS}; do
    ...
    OS_ENDPOINT_ID=$( openstack endpoint list  -f csv --quote none | \
                      grep "^[a-z0-9]*,${AUTH_ZUN_KEYSTONE_REGION},${OS_SERVICE_NAME},${OS_SERVICE_TYPE},True,${CURRENT_OS_SVC_ENDPOINT_IFACE}," | \
                      awk -F ',' '{ print $1 }' )
    ...
    if [[ "${OS_ENDPOINT_UPDATE}" == "True" ]]; then
      OS_ENDPOINT_ID=$( openstack endpoint create -f value -c id \
        --region="${AUTH_ZUN_KEYSTONE_REGION}" \
        "${OS_SERVICE_ID}" \
