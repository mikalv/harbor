...
if ! [ $OS_MANAGEMENT_ACTION == "bootstrap" ]; then
  ...
  ##############################################################################
  /opt/harbor/mistral/manage/bootstrap-database.sh


  echo "${OS_DISTRO}: Managing Users"
  ##############################################################################
  /opt/harbor/mistral/manage/manage-keystone-user.sh


  echo "${OS_DISTRO}: Managing Service"
  ##############################################################################
  /opt/harbor/mistral/manage/manage-keystone-service.sh
  ...
  ##############################################################################
  /opt/harbor/mistral/bootstrap/bootstrap-images.sh
