...
for OS_SERVICE_NAME in mistral; do
  ...
  if [ "$OS_SERVICE_NAME" == "mistral" ]; then
    ...
    OS_SVC_ENDPOINT_INTERNAL="https://${MISTRAL_API_SERVICE_HOST_SVC}/v2"
    OS_SVC_ENDPOINT_ADMIN="https://${MISTRAL_API_SERVICE_HOST_SVC}:${MISTRAL_API_SVC_PORT}/v2"
    OS_SVC_ENDPOINT_PUBLIC="https://${MISTRAL_API_SERVICE_HOST}/v2"
  fi
