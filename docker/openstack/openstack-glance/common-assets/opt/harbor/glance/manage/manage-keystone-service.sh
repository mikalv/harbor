...
OS_SERVICE_TYPE="image"
OS_SERVICE_DESC="${OS_DOMAIN}: ${OS_SERVICE_NAME} (${OS_SERVICE_TYPE}) service"
OS_SVC_ENDPOINTS="PUBLIC ADMIN INTERNAL"
OS_SVC_ENDPOINT_INTERNAL="https://${GLANCE_API_SERVICE_HOST_SVC}:${GLANCE_API_SVC_PORT}"
OS_SVC_ENDPOINT_ADMIN="https://${GLANCE_API_SERVICE_HOST_SVC}:${GLANCE_API_SVC_PORT}"
OS_SVC_ENDPOINT_PUBLIC="https://${GLANCE_API_SERVICE_HOST}"
...
for OS_SVC_ENDPOINT in ${OS_SVC_ENDPOINTS}; do
  ...
  OS_ENDPOINT_ID=$( openstack endpoint list  -f csv --quote none | \
                    grep "^[a-z0-9]*,${AUTH_GLANCE_KEYSTONE_REGION},${OS_SERVICE_NAME},${OS_SERVICE_TYPE},True,${CURRENT_OS_SVC_ENDPOINT_IFACE}," | \
                    awk -F ',' '{ print $1 }' )
  ...
  if [[ "${OS_ENDPOINT_UPDATE}" == "True" ]]; then
    OS_ENDPOINT_ID=$( openstack endpoint create -f value -c id \
      --region="${AUTH_GLANCE_KEYSTONE_REGION}" \
      "${OS_SERVICE_ID}" \
