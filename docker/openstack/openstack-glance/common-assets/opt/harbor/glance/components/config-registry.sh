...
set -e
echo "${OS_DISTRO}: Configuring Glance Registry"
################################################################################
...
check_required_vars GLANCE_CONFIG_FILE \
                    OS_DOMAIN \
                    GLANCE_REGISTRY_SVC_PORT \
                    MY_IP \
                    API_WORKERS \
                    GLANCE_REGISTRY_TLS_CA \
                    GLANCE_REGISTRY_TLS_CERT \
                    GLANCE_REGISTRY_TLS_KEY \
                    GLANCE_REGISTRY_SERVICE_HOST_SVC
...
echo "${OS_DISTRO}:    Port: ${GLANCE_REGISTRY_SVC_PORT}"
echo "${OS_DISTRO}:    Listen: ${MY_IP}"
crudini --set ${GLANCE_CONFIG_FILE} DEFAULT bind_port "${GLANCE_REGISTRY_SVC_PORT}"
...
CERT_MOD="$(openssl x509 -noout -modulus -in ${GLANCE_REGISTRY_TLS_CERT})"
KEY_MOD="$(openssl rsa -noout -modulus -in ${GLANCE_REGISTRY_TLS_KEY})"
...
################################################################################
crudini --set ${GLANCE_CONFIG_FILE} DEFAULT cert_file "${GLANCE_REGISTRY_TLS_CERT}"
crudini --set ${GLANCE_CONFIG_FILE} DEFAULT key_file "${GLANCE_REGISTRY_TLS_KEY}"
